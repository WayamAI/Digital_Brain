...
import { Btn, DataTable, Drawer, ExportBtn, Filters, KeyVals, Kpi, Meter, Panel, Pill } from "@/components/kit";
  ...
  return (
    ...
    >
      <div className="grid gap-3 sm:grid-cols-4">
        ...
        <Kpi label="Change success rate (30d)" value="96.2%" sub="12 failures" tone="ok" trend="↑ 1.4%" />
      ...
      <Panel className="mt-4" title="Upcoming changes" desc="Click a change for history, dependencies and rollback readiness" pad={false}>
        <div className="px-4 pt-3">
          <Filters
            groups={[{ key: "rec", label: "Recommendation", options: ["Proceed", "Reschedule", "Add rollback plan"] }]}
            state={f}
            onChange={(k, v) => setF((s) => ({ ...s, [k]: v }))}
            right={<span className="text-[11px] text-muted-foreground">{rows.length} changes</span>}
          />
        </div>
        <DataTable<C>
          rows={rows}
          ...
          onRow={setSel}
          cols={[
            { key: "id", header: "Change", cell: (r) => <span className="font-medium">{r.id}</span> },
            { key: "system", header: "System" },
            { key: "desc", header: "Description", cell: (r) => <span className="text-muted-foreground">{r.desc}</span> },
            { key: "window", header: "Requested window" },
            {
              key: "risk",
              header: "Predicted failure risk",
              cell: (r) => <Meter value={r.risk} tone={r.risk >= 40 ? "crit" : r.risk >= 25 ? "warn" : "ok"} />,
            },
      ...
      <Drawer open={!!sel} onClose={() => setSel(null)} title={sel ? `${sel.id} · ${sel.system}` : ""} subtitle={sel?.desc}>
        {sel && (
          <>
            <KeyVals
              items={[
                ["Requested window", sel.window],
                ["Predicted failure risk", <Meter key="m" value={sel.risk} tone={sel.risk >= 40 ? "crit" : sel.risk >= 25 ? "warn" : "ok"} />],
                ["Recommendation", <Pill key="r">{sel.rec}</Pill>],
                ["Approver", sel.approver],
              ]}
            />
            <p className="rounded-md bg-muted px-3 py-2 text-[12.5px] leading-relaxed">{sel.note}</p>
            ...
            <section>
              ...
              <ul className="space-y-1.5 text-[12.5px]">
                <li className="flex justify-between rounded-md border border-border px-3 py-2">
                  ...
                  <span className="text-crit">3 of 11 failed (27%)</span>
                </li>
                <li className="flex justify-between rounded-md border border-border px-3 py-2">
                  <span>Same system, off peak window</span>
                  <span className="text-ok">0 of 14 failed (0%)</span>
                </li>
                <li className="flex justify-between rounded-md border border-border px-3 py-2">
                  <span>Same change type, all systems (90d)</span>
                  <span className="text-warn-ink">5 of 62 failed (8%)</span>
                </li>
              </ul>
            </section>
            ...
            <section>
              ...
              <p className="text-[12.5px] leading-relaxed">
                Touches 1 Tier 0 service and 5 downstream consumers, including Warehouse Management (41
                NA distribution centres) and Beverages e Commerce. Aggregate exposure during the window:
                approximately $1.2M/hour if the change fails without rollback.
              </p>
            </section>
            ...
            <section>
              ...
              <div className="rounded-md bg-ok-soft px-3 py-2 text-[12.5px] text-ok">
                Suggested: Aug 17, 02:00–05:00 CT (off peak, no competing changes, full L2 coverage).
              </div>
              <ul className="mt-2 space-y-1 text-[12.5px]">
                {[
                  ...
                  <li key={String(t)} className="flex items-center gap-2">
                    <span className={`h-2 w-2 rounded-full ${ok ? "bg-ok" : "bg-warn"}`} />
                    {t}
                  </li>
                ))}
              </ul>
            ...
            <div className="flex gap-2 border-t border-border pt-3">
              <Btn variant="ok" onClick={() => toast.success(`${sel.id} approved for the suggested window`)}>
                Accept recommendation
              </Btn>
              <Btn variant="outline" onClick={() => toast.message(`${sel.id} sent to CAB for review`)}>
                Send to CAB
              </Btn>
            </div>
          </>
