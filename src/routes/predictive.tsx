  ...
  head: () => ({
    meta: [
      ...
      { property: "og:title", content: "Predictive Operations — Digital Brain" },
      {
        ...
        content:
          "Two predicted outages, four SLA breaches and three bottlenecks in the next seven days.",
      },
    ],
  }),
  ...
  return (
    <AppShell
      ...
      actions={
        <>
          <Btn
            variant="outline"
            onClick={() => run("Forecast recomputed on the last 90 days of signals")}
          >
            {running ? <AppIcon name="loading" size="sm" spin /> : null} Re run forecast
          </Btn>
    ...
    >
      <div className="grid gap-3 sm:grid-cols-3">
        <Kpi
          label="Predicted outages (next 7d)"
          value="2"
          sub="SAP OM, Azure AD EMEA"
          tone="crit"
        />
        <Kpi label="Predicted SLA breaches" value="4" sub="2 within 48 hours" tone="warn" />
        <Kpi
          label="Predicted capacity bottlenecks"
          value="3"
          sub="earliest in 4 weeks"
          tone="info"
        />
      </div>
      ...
      <Panel
        ...
        right={
          ranAt ? <span className="text-2xs text-tertiary">recomputed {ranAt}</span> : undefined
        }
      >
        ...
        <p className="mt-2 text-2xs text-tertiary">
          Mean absolute error 2.1 incidents per period · model retrained weekly on closed incident
          outcomes.
        </p>
      </Panel>
          ...
          cols={[
            {
              key: "p",
              header: "Prediction",
              cell: (r) => <span className="font-medium">{r.p}</span>,
            },
            {
              key: "conf",
              header: "Confidence",
              cell: (r) => (
                <Meter
                  value={r.conf}
                  tone={r.conf >= 75 ? "crit" : r.conf >= 60 ? "warn" : "info"}
                />
              ),
            },
            ...
            {
              key: "signals",
              header: "Driving signals",
              cell: (r) => <span className="text-tertiary">{r.signals}</span>,
            },
