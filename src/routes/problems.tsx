  ...
  return (
    ...
    >
      <div className="grid gap-3 sm:grid-cols-3">
        <Kpi label="Recurring patterns detected (30d)" value="9" sub="across 214 services" tone="info" />
        <Kpi label="Problem records auto opened" value="6" sub="3 pending engineering triage" tone="warn" />
        <Kpi label="Permanent fixes shipped" value="3" sub="14 incidents/mo prevented" tone="ok" trend="↑ 2" />
      </div>

      <Panel className="mt-4" title="Problem records" desc="Click a record for the clustered incidents and root cause writeup" pad={false}>
        <DataTable<P>
          rows={problems}
          rowKey={(r) => r.id}
          onRow={setSel}
          cols={[
            { key: "id", header: "Problem", cell: (r) => <span className="font-medium">{r.id}</span> },
            { key: "pattern", header: "Pattern" },
            { key: "occ", header: "Occurrences (30d)", align: "right" },
            { key: "service", header: "Affected service" },
            { key: "fix", header: "Recommended permanent fix", cell: (r) => <span className="text-muted-foreground">{r.fix}</span> },
            { key: "status", header: "Status", cell: (r) => <Pill>{r.status}</Pill> },
          ]}
        />
      </Panel>

      <Drawer open={!!sel} onClose={() => setSel(null)} title={sel ? `${sel.id} · ${sel.pattern}` : ""} subtitle={sel?.service}>
        {sel && (
          <>
            <KeyVals
              items={[
            ...
            <section>
              ...
              <p className="rounded-md bg-muted px-3 py-2 text-[12.5px] leading-relaxed">{sel.writeup}</p>
            </section>
            <section>
              <h4 className="mb-2 text-[12px] font-semibold uppercase tracking-wide text-muted-foreground">
                Clustered incidents
              ...
              <ol className="relative space-y-2 border-l border-border pl-4">
                {sel.incidents.map((i, idx) => (
                  <li key={i} className="relative">
                    ...
                    <span className="text-[12.5px] font-medium">{i}</span>
